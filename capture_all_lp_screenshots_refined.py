...
async def run():
    async with async_playwright() as p:
        ...
        for name, path in pages:
            ...
            print(f"Capturando REFINED {name} em {url}...")
            try:
                await page.goto(url)
                await page.wait_for_load_state("networkidle")
                # Wait a bit for the cookie consent to appear
                await asyncio.sleep(2)
                await page.screenshot(path=f"refined_preview_{name}.png", full_page=True)
